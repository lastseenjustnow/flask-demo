...
def getCursor(dr, s, db, u, p):
    ...
    return pyodbc.connect(params).cursor()
...
dateparser = lambda x: datetime.datetime.strptime(x, '%d/%m/%Y')
...
# client_code_absence!
client_code_absence = input_data.merge(client_master, how="left", left_on="Client_info", right_on="ClientCode")
client_code_absence[client_code_absence.ClientCode.isnull()]['Client_info']
